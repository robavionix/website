...
export const useTranslations = async (
  ...
  const menuConfig = menu.default || menu;
  const dictionaryConfig = dictionary.default || dictionary;

  // Keep raw menus available when dictionary keys share the same name.
  const translations = {
    ...menuConfig,
    ...dictionaryConfig,
    menuConfig,
