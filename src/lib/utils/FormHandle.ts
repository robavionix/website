...
export function formReset(form: HTMLFormElement) {
  ...
  validationTags?.forEach((tag) => {
    tag.classList.remove("success", "error", "is-filled", "is-error");
  });
...
export function isFormFilled(form: HTMLFormElement): boolean {
  const elements = form.querySelectorAll(
    "input[name], [input-wrapper]:not(.hidden) select[data-hs-select], select[name], textarea[name]",
  );
  ...
  for (let element of elements) {
    ...
    if (
      ...
      if (checkedGroups.has(elem.name)) {
        continue;
      }

      checkedGroups.add(elem.name);

      if (!form.querySelector(`[name="${elem.name}"]:checked`)) {
        return false;
      }
    } else if (elem.value === "") {
      return false;
...
export const setMessage = (
  ...
  const messageType =
    success === "pending" ? "pending" : success ? "success" : "error";
  const allMessages = form.querySelectorAll(".message");
  const messageElement = form.querySelector(`.message.${messageType}`);
  const msgEleText =
    messageElement?.querySelector(".prose-styles") ||
    messageElement?.querySelector(".contact-form-message-content");
...
export const formspreeSubmit = async (
  ...
  try {
    ...
    setMessage(
      error + "! Please use the configured contact email to submit a ticket.",
      true,
      false,
      form,
    );
